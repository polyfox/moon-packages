##
# :nodoc:
module Moon
  ##
  # Basic Assertion Framework
  module Test
    module Assert
      class AssertError < StandardError
        #
      end

      ##
      # @return [Void]
      private def init_assertions
        @assertions = 0
        @assertions_raised = 0
      end

      ##
      # @param [String] msg
      #   @optional
      # @param [Exception] original_exception
      #   @optional
      private def raise_assert_error(msg = nil, original_exception = nil)
        @assertions_raised += 1
        if original_exception
          fail AssertError, (msg || "assertion failed\n") + original_exception.inspect + "\n" + original_exception.backtrace("\n").indent(2)
        else
          fail AssertError, (msg || 'assertion failed')
        end
      end

      ##
      # @param [Object] obj
      # @param [String] msg
      #   @optional
      def assert(obj, msg = nil)
        @assertions += 1
        obj ? true : raise_assert_error(msg)
      end

      ##
      # @param [Object] obj
      # @param [String] msg
      #   @optional
      def assert_true(obj, msg = nil)
        assert(!!obj, msg)
      end

      ##
      # @param [Object] obj
      # @param [String] msg
      #   @optional
      def assert_false(obj, msg = nil)
        assert_true(!obj, msg)
      end

      ##
      # @param [Object] a
      # @param [Object] b
      # @param [String] msg
      #   @optional
      def assert_equal(a, b, msg = nil)
        assert_true(a == b, msg || "expected #{a} == #{b}")
      end

      ##
      # @param [Object] a
      # @param [Object] b
      # @param [String] msg
      #   @optional
      def assert_not_equal(a, b, msg = nil)
        assert_true(a != b, msg || "expected #{a} != #{b}")
      end

      ##
      # @param [Object] a
      # @param [Object] b
      # @param [String] msg
      #   @optional
      def assert_same(a, b, msg = nil)
        assert_true(a.equal?(b), msg || "expected #{a} to #equal? #{b}")
      end

      ##
      # @param [Object] a
      # @param [Object] b
      # @param [String] msg
      #   @optional
      def assert_not_same(a, b, msg = nil)
        assert_false(a.equal?(b), msg || "expected #{a} to not #equal? #{b}")
      end

      ##
      # @param [Object] a
      # @param [String] msg
      #   @optional
      def assert_nil(a, msg = nil)
        assert_equal(a, nil, msg)
      end

      ##
      # @param [Class<Exception>] klass
      # @param [String] msg
      #   @optional
      # @yield
      def assert_raise(klass, msg = nil)
        begin
          yield
        rescue Exception => e
          assert_equal(e.class, klass, msg)
        end
      end

      ##
      # @param [String] msg
      #   @optional
      # @yield
      def assert_not_raise(msg = nil)
        @assertions += 1
        begin
          yield
        rescue Exception => e
          raise_assert_error(msg || 'expected no raise', e)
        end
      end

      ##
      # @param [Class] klass
      # @param [Object] obj
      # @param [String] msg
      #   @optional
      def assert_kind_of(klass, obj, msg = nil)
        assert_true(obj.kind_of?(klass), msg || "expected #{obj} to be kind_of? #{klass}")
      end

      ##
      # @param [Class] klass
      # @param [Object] obj
      # @param [String] msg
      #   @optional
      def assert_not_kind_of(klass, obj, msg = nil)
        assert_false(obj.kind_of?(klass), msg || "expected #{obj} not to be kind_of? #{klass}")
      end

      ##
      # @param [Float] exp
      # @param [Float] act
      # @param [String] msg
      #   @optional
      def assert_float(exp, act, msg = nil)
        assert_true(check_float(exp, act), msg || "expected #{exp} to equal #{act}")
      end

      ##
      # Swiped from mruby/test/assert
      # Performs fuzzy check for equality on methods returning floats
      #
      # @param [Float] a
      # @param [Float] b
      def check_float(a, b)
        #tolerance = 1e-12
        tolerance = 1e-2
        a = a.to_f
        b = b.to_f
        if a.finite? and b.finite?
          (a-b).abs < tolerance
        else
          true
        end
      end

      # extended
      def assert_ary_size(expected, ary, msg = nil)
        assert_equal(expected, ary.size, msg || "expected Array of size #{expected}")
      end
    end
    include Assert
  end
end
